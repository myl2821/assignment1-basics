...
from cs336_basics.model.pipeline import scaled_dot_product_attention, CausalMultiHeadSelfAttention, TransformerBlock
...
def run_linear(
    ...
    layer = Linear(d_in, d_out, weights)
...
def run_embedding(
    ...
    layer = Embedding(vocab_size, d_model, weights)
...
def run_multihead_self_attention(
    ...
    layer.W_q.weight.data = q_proj_weight
    layer.W_k.weight.data = k_proj_weight
    layer.W_v.weight.data = v_proj_weight
    layer.W_o.weight.data = o_proj_weight
...
def run_multihead_self_attention_with_rope(
    ...
    rope_layer = RotaryPositionalEmbedding(theta, d_head, max_seq_len)

    att_layer = CausalMultiHeadSelfAttention(d_model, num_heads)
    att_layer.W_q.weight.data = q_proj_weight
    att_layer.W_k.weight.data = k_proj_weight
    att_layer.W_v.weight.data = v_proj_weight
    att_layer.W_o.weight.data = o_proj_weight

    return att_layer.forward(in_features, rope_layer, token_positions)
